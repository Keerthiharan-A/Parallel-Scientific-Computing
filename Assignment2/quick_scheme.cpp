...
int main(int argc, char *argv[]) {
    ...
    for (int i = 1; i <= t; ++i) {
        ...
        // Perform Quick scheme
        u[i][1] = u[i-1][1] - (delt / delx) * (u[i-1][1] - u[i-1][0]);
        for (int j = 2; j <= n_local; ++j) {
            u[i][j] = u[i-1][j] - (delt / delx) * ((3.0/8.0) * u[i-1][j] - (7.0/8.0) * u[i-1][j - 1] + (1.0/8.0) * u[i-1][j - 2] + (3.0/8.0) * u[i-1][j + 1]);
        }
    }
    ...
    if (rank == 0) {
        cout << "Numerical solution at t = 0.5 using Quick scheme:\n\n";
        for (int i = 0; i <= n; ++i) {
            cout << u_global_1[i] << " ";
        }
        cout << "\n";
        cout << "Numerical solution at t = 1 using Quick scheme:\n\n";
        for (int i = 0; i <= n; ++i) {
            cout << u_global_2[i] << " ";
        }
        cout << "\n";
